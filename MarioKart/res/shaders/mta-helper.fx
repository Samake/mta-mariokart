...
float4x4 gLightView : LIGHTVIEW;
float4x4 gLightProjection : LIGHTVIEWPROJECTION;
...
float4 MTACalcLightPosition( float3 InPosition )
{
    float4 posWorld = mul(float4(InPosition,1), gWorld);
    float4 posWorldView = mul(posWorld, gLightView);
    return mul(posWorldView, gLightProjection);
}
...
//------------------------------------------------------------------------------------------
// MTACalcGTALights
// - Calc original MTA lights
//------------------------------------------------------------------------------------------
float4 MTACalcGTALights(float4 InDiffuse)
{
...
//------------------------------------------------------------------------------------------
// MTACalcGTAVehicleDiffuse
// - Calculate dynamic GTA lighting for vehicles
//------------------------------------------------------------------------------------------
float4 MTACalcGTAVehicleDynamicDiffuse( float3 WorldNormal, float4 InDiffuse, float3 lightDirection)
{
    // Calculate diffuse color by doing what D3D usually does
    float4 ambient  = gAmbientMaterialSource  == 0 ? gMaterialAmbient  : InDiffuse;
    float4 diffuse  = gDiffuseMaterialSource  == 0 ? gMaterialDiffuse  : InDiffuse;
...
// - Calculate GTA lighting  (all 4 lights)
//------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------
// MTACalculateSpecular2
// - Get specular intensity
//------------------------------------------------------------------------------------------
float MTACalculateSpecular2(float3 LightDir, float3 SurfNormal, float SpecPower)
{
    // Using Blinn half angle modification for performance over correctness
    LightDir = normalize(LightDir);
    SurfNormal = normalize(SurfNormal);
    float3 halfAngle = normalize(-gCameraDirection - LightDir);
    float r = dot(halfAngle, SurfNormal);
    return pow(saturate(r), SpecPower);
}
...
//------------------------------------------------------------------------------------------
// MTACalculateVehicleSpecular2
// - Get specular intensity
//------------------------------------------------------------------------------------------
float4 MTACalculateVehicleSpecular2(float3 SurfNormal, float3 LightDir)
{
    // Using Blinn half angle modification for performance over correctness
    SurfNormal = normalize(SurfNormal);
    float3 halfAngle = normalize(-gCameraDirection - LightDir);
...
// Pack
//------------------------------------------------------------------------------------------
float3 Pack(float buffer)
{
    float b = frac(buffer); 
    float c = frac(b * 255);
    b -= c/255;
    float a = (buffer - b) / 255;
    return float3(a,b,c);
}
...
// Pack
//------------------------------------------------------------------------------------------
float Unpack(float3 color)
{
    return float(color.r * 255 + color.g + color.b / 255);
}
...
// CreateDirLight
//------------------------------------------------------------------------------------------
float createDirLight(float3 Normal, float3 LightDir)
{		 
