...
{	
    ...
    float4 dynamicLightColor = float4(ambientColor.rgb * ambientIntensity, 1) + float4((diffuseColor.rgb * diffuseIntensity)* sunColor.rgb, 1) + saturate(finalSpecular * specularIntensity);
	dynamicLightColor.rgb *= 0.5;
	dynamicLightColor.a = mainColor.a;
	
	return dynamicLightColor * mainColor;
}
