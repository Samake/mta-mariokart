...
function(resource)
	if (resource == getThisResource()) then
		for i = 1, maxObjects do
			objectTexture = engineLoadTXD("models/peach.txd")
			objectModel = engineLoadDFF("models/peach" .. i .. ".dff", objectID + i)
			objectCol = engineLoadCOL("models/peach" .. i .. ".col")
			...
			if (not objectTexture) then
				outputChatBox("Load texture failed!")
			end
		...
		local sound = playSound("sounds/peach_beach.ogg", true)
